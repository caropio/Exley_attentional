...
self_lottery_differences_all_censored  = pd.DataFrame(columns=['number', 'prob_option_A'])

for i in self_lottery_censored ['number'].unique():
    individual = self_lottery_censored.loc[self_lottery_censored ['number'] == i, ['case', 'prob_option_A', 'valuation', 'dwell_time_relative']] 
    individual_difference = individual.pivot(index='prob_option_A', columns='case')
    individual_difference['valuation_ASPS_ACPS'] = individual_difference['valuation']['ASPS'] - individual_difference['valuation']['ACPS']
    individual_difference['dwell_time_ASPS_ACPS'] = individual_difference['dwell_time_relative']['ASPS'] - individual_difference['dwell_time_relative']['ACPS']
    individual_difference['number'] = i
    individual_difference.reset_index(inplace=True)
    individual_difference.columns = individual_difference.columns.droplevel(1)
    self_lottery_differences_all_censored = pd.concat([self_lottery_differences_all_censored, individual_difference[['number', 'prob_option_A', 'valuation_ASPS_ACPS', 'dwell_time_ASPS_ACPS']]], ignore_index=True)

charity_lottery_differences_all_censored  = pd.DataFrame(columns=['number', 'prob_option_A'])

for i in charity_lottery_censored ['number'].unique():
    individual = charity_lottery_censored.loc[charity_lottery_censored['number'] == i, ['case', 'prob_option_A', 'valuation', 'dwell_time_relative']] 
    individual_difference = individual.pivot(index='prob_option_A', columns='case')
    individual_difference['valuation_ACPC_ASPC'] = individual_difference['valuation']['ACPC'] - individual_difference['valuation']['ASPC']
    individual_difference['dwell_time_ACPC_ASPC'] = individual_difference['dwell_time_relative']['ACPC'] - individual_difference['dwell_time_relative']['ASPC']
    individual_difference['number'] = i
    individual_difference.reset_index(inplace=True)
    individual_difference.columns = individual_difference.columns.droplevel(1)
    charity_lottery_differences_all_censored = pd.concat([charity_lottery_differences_all_censored, individual_difference[['number', 'prob_option_A', 'valuation_ACPC_ASPC', 'dwell_time_ACPC_ASPC']]], ignore_index=True)
...
# plt.title('(Within-subj) Diff SELF Valuation x Dwell time')
# plt.grid(True)
# plt.show()
...
# plt.xlabel('Dwell time')
# plt.ylabel('Valuation')
# plt.title('EDRP Diff SELF Valuation x Dwell time')
# plt.grid(True)
# plt.show()
...
self_lottery_differences_all_EDRP_censored = pd.concat([self_lottery_differences_all_EDRP, self_lottery_differences_all_censored], ignore_index=True)
charity_lottery_differences_all_EDRP_censored = pd.concat([charity_lottery_differences_all_EDRP, charity_lottery_differences_all_censored], ignore_index=True)

md_self_EDRP_censored = smf.mixedlm("valuation_ASPS_ACPS ~ dwell_time_ASPS_ACPS", self_lottery_differences_all_EDRP_censored, groups=self_lottery_differences_all_EDRP_censored["number"])
mdf_self_EDRP_censored = md_self_EDRP_censored.fit()
print(mdf_self_EDRP_censored.summary())

md_charity_EDRP_censored = smf.mixedlm("valuation_ACPC_ASPC ~ dwell_time_ACPC_ASPC", charity_lottery_differences_all_EDRP_censored, groups=charity_lottery_differences_all_EDRP_censored["number"])
mdf_charity_EDRP_censored = md_charity_EDRP_censored.fit()
print(mdf_charity_EDRP_censored.summary())


# ALL AND CENSORED 

self_lottery_differences_all_all_censored = pd.concat([self_lottery_differences_all, self_lottery_differences_all_censored], ignore_index=True)
charity_lottery_differences_all_all_censored = pd.concat([charity_lottery_differences_all, charity_lottery_differences_all_censored], ignore_index=True)

md_self_all_censored = smf.mixedlm("valuation_ASPS_ACPS ~ dwell_time_ASPS_ACPS", self_lottery_differences_all_all_censored, groups=self_lottery_differences_all_all_censored["number"])
mdf_self_all_censored = md_self_all_censored.fit()
print(mdf_self_all_censored.summary())

md_charity_all_censored = smf.mixedlm("valuation_ACPC_ASPC ~ dwell_time_ACPC_ASPC", charity_lottery_differences_all_all_censored, groups=charity_lottery_differences_all_all_censored["number"])
mdf_charity_all_censored = md_charity_all_censored.fit()
print(mdf_charity_all_censored.summary())
